<div class="footer_secrion">
    <div class="container">
        <div class="footer">
            <div class="props">
                <p>ООО "Охрана и безопасность труда"</p>
                <p>ИНН: 7606113196</p>
                <p>КПП: 760601001</p>
                <p>ОГРН: 1177627015477</p>
            </div>
            <div class="footer_services">
                <a href="#" target="_blank"><img src="/img/mintrud.png" alt="Министерство труда и социальной защиты России" width="165px"></a>
                <a href="#" target="_blank"><img src="/img/rosakr_bottom.png" alt="Росаккредитация" width="165px"></a>
                <a href="#" target="_blank"><img src="/img/fstz.png" alt="Федеральная служба по труду и занятости" width="165"></a>
                <a href="#" target="_blank"><img src="/img/soc-strah.png" alt="Фонд социального страхования" width="165"></a>
            </div>
            <div class="footer_contacts">
                <p class="ftel">+7(4852) 73-72-68</p>
                <p class="ftel">+7(4852) 73-86-58</p>
                <p class="faddr">г. Ярославль, пк-т Октября, дом 88, офис 102, пом. 2.</p>
                <p class="fmail">info@caot.ru</p>
            </div>
        </div>
        <div class="copyright">
            <p><a href="#">Политика конфиденциальности</a></p>
            <p class="dev">Разработка сайта - <a href="#">AIDA</a></p>
            <p>&copy 2016-2017</p>
        </div>
    </div>
</div>

<!-- Optional JavaScript -->
<!-- jQuery first, then Popper.js, then Bootstrap JS -->
<script src="https://code.jquery.com/jquery-3.2.1.slim.min.js"
        integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN"
        crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.3/umd/popper.min.js"
        integrity="sha384-vFJXuSJphROIrBnz7yo7oB41mKfc8JzQZiCq4NCceLEaO4IHwicKwpJf9c9IpFgh"
        crossorigin="anonymous"></script>
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/js/bootstrap.min.js"
        integrity="sha384-alpBpkh1PFOepccYVYDB4do5UnbKysX5WZXm3XxPqe5iKTfUKjNkCk9SaVuEZflJ"
        crossorigin="anonymous"></script>
</body>
</html>